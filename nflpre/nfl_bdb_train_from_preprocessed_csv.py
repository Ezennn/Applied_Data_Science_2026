...
BASE_FILENAME = "nfl_bdb_gnn_complete_v9_evidence_weighted.py"
...
def _resolve_base_path() -> str:
    ...
    try:
        ...
        candidates.extend(sorted(glob.glob(os.path.join(here, "nfl_bdb_gnn_complete_v9_evidence_weighted*.py"))))
    except Exception:
        pass

    candidates.extend([
        os.path.join(os.getcwd(), BASE_FILENAME),
        *sorted(glob.glob(os.path.join(os.getcwd(), "nfl_bdb_gnn_complete_v9_evidence_weighted*.py"))),
        f"/content/{BASE_FILENAME}",
        *sorted(glob.glob("/content/nfl_bdb_gnn_complete_v9_evidence_weighted*.py")),
        f"/mnt/data/{BASE_FILENAME}",
        *sorted(glob.glob("/mnt/data/nfl_bdb_gnn_complete_v9_evidence_weighted*.py")),
    ])
...
def build_samples_from_preprocessed_dir(
    ...
    train_pairs = discover_preprocessed_train_pairs(preprocessed_dir)
...
def evaluate_fast(model, loader, device, config, loss_controller, amp_enabled: bool):
    ...
    return base.aggregate_metrics(metric_list)
...
def train_model_fast_resume(
    ...
    for epoch in range(start_epoch, config.num_epochs):
        ...
        print(
            f"Epoch {epoch + 1:02d} | train {train_metrics.get('loss', float('nan')):.5f} | "
            f"val {val_loss if run_val else float('nan'):.5f} | "
            f"track {train_metrics.get('track_loss_raw', float('nan')):.5f} | "
            f"vel {train_metrics.get('vel_loss_raw', float('nan')):.5f}"
        )

        if run_val and val_loss < best_val - 1e-4:
            best_val = val_loss
            best_epoch = epoch
...
def predict_from_preprocessed_test_csv(
    ...
    test_input = pd.read_csv(test_input_csv)

    input_groups = test_input.groupby(["game_id", "play_id"], sort=False).indices
    ...
    if template_path is not None:
        ...
        merged = template.merge(full_df, on=required, how="left")
        merged.to_csv(output_path, index=False)
        return merged, full_df, bundle

    return full_df, full_df, bundle
