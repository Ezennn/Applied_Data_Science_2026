...
def export_preprocessed_csvs(
    ...
    """
    Read the original competition CSV files, preprocess them week by week,
    and write standalone weekly preprocessed CSV files.
    """
    ...
    if not train_input_paths or not train_output_paths:
        raise FileNotFoundError(
            "Could not find train_input and train_output CSV files under data_dir."
        )

    train_pairs = _pair_paths(train_input_paths, train_output_paths)
    if not train_pairs:
        raise FileNotFoundError("Could not pair any weekly train_input/train_output CSV files.")
